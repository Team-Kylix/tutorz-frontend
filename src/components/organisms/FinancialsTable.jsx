...
/**
 * Renders the Institute's class payment history table.
 * Mirrors TutorFinancialsTable structure/theme — with PDF download via RowActions.
 *
 * Props:
 *   payments — array of ClassPaymentHistoryDto from /api/payment/class/history
 */
const FinancialsTable = ({ payments = [] }) => {
    ...
    const handleDownload = async (payment) => {
        ...
        try {
            ...
            await downloadInstitutePaymentPdf(payment.paymentId, ref);
        } finally {
                ...
                <table className="w-full text-sm text-left whitespace-nowrap">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {payments.map((payment, idx) => {
                            ...
                            // Tutor name — may come as tutorName or tutorFirstName + tutorLastName
                            const tutorName = payment.tutorName
                                || (payment.tutorFirstName || payment.tutorLastName
                                    ? `${payment.tutorFirstName || ''} ${payment.tutorLastName || ''}`.trim()
                                    : null);
                                                ...
                                                <div className="flex items-center text-[10px] md:text-xs text-indigo-500 dark:text-indigo-400 mt-0.5 gap-1">
                                                    <span className="truncate max-w-[200px] md:max-w-none">
                                                        ...
                                                        {payment.subject && payment.subject !== payment.className
                                                            ...
                                                            : ''}
                                                        {tutorName && (
                                                            <span className="text-gray-400 dark:text-gray-500"> · {tutorName}</span>
                                                        )}
                                                    </span>
                                                </div>
                                            )}
